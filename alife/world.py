# World module for ALife simulation
...
class World:
    """
    Game world containing agents and food.
    Handles simulation updates and entity management.
    """
    ...
    def __init__(self):
        ...
        for _ in range(config.FOOD_MAX):
            self.spawn_food()
    ...
    def spawn_food(self):
        pos = np.array(
            [
                random.uniform(10.0, config.WORLD_W - 10.0),
                random.uniform(10.0, config.WORLD_H - 10.0),
            ],
            dtype=np.float32,
        )
        self.foods.append(
    ...
    def spawn_random_agent(self):
        ...
        pos = np.array(
            [
                random.uniform(30.0, config.WORLD_W - 30.0),
                random.uniform(30.0, config.WORLD_H - 30.0),
            ],
            dtype=np.float32,
        )
    ...
    def update(self):
        ...
        if len(self.foods) < config.FOOD_MAX and random.random() < config.FOOD_RESPAWN:
            self.spawn_food()
        ...
        if len(self.agents) < config.MIN_AGENTS:
            for _ in range(config.MIN_AGENTS - len(self.agents)):
                self.spawn_random_agent()


# Import math here to avoid circular dependency issues
import math
